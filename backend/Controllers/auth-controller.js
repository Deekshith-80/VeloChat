...
const sendotp = async (req, res) => {
  try {
    ...
    let mailDetails = {
      from: `"VeloChat" <${EMAIL}>`,
      to: email,
      subject: "Your VeloChat Login OTP - " + otp,
      html: `<!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Your VeloChat OTP</title>
  </head>
  <body style="margin:0;padding:0;background-color:#f4f7f5;font-family:'Segoe UI',Helvetica,Arial,sans-serif;">
    ...
    <tr>
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <!-- Header -->
        <tr>
        <td align="center" style="background:linear-gradient(135deg,#064e3b 0%,#10b981 100%);padding:36px 40px;">
          <h1 style="margin:0;color:#ffffff;font-size:26px;font-weight:700;letter-spacing:0.5px;">VeloChat</h1>
        ...
        <td style="padding:40px 40px 32px;">
          ...
          We received a request to sign in to your VeloChat account. Use the one-time password below to complete your login.
          </p>

          <!-- OTP Box -->
          <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td align="center" style="background-color:#ecfdf5;border:2px dashed #10b981;border-radius:10px;padding:24px;">
            <p style="margin:0 0 6px;font-size:12px;color:#6b7280;letter-spacing:1px;text-transform:uppercase;">One-Time Password</p>
            <p style="margin:0;font-size:42px;font-weight:800;letter-spacing:10px;color:#10b981;">${otp}</p>
            </td>
          </tr>
          </table>
        ...
        <td style="padding:0 40px 32px;">
          <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="background-color:#f0fdf4;border-left:4px solid #10b981;border-radius:0 6px 6px 0;padding:12px 16px;">
        ...
        <td align="center" style="background-color:#f9fafb;border-top:1px solid #e5e7eb;padding:20px 40px;">
          ...
          &copy; ${new Date().getFullYear()} VeloChat. All rights reserved.
          </p>
...
const sendVerificationOtp = async (req, res) => {
  try {
    ...
    const mailDetails = {
      from: `"VeloChat" <${EMAIL}>`,
      to: user.email,
      subject: `Verify your VeloChat email – OTP: ${otp}`,
      html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
        ...
        <table width="520" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              ...
              <p style="margin:0 0 28px;font-size:15px;color:#374151;line-height:1.6;">
                Please use the OTP below to verify your email address and unlock full access to VeloChat.
              </p>
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="background-color:#ecfdf5;border:2px dashed #10b981;border-radius:10px;padding:24px;">
            ...
            <td style="padding:0 40px 32px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="background-color:#f0fdf4;border-left:4px solid #10b981;border-radius:0 6px 6px 0;padding:12px 16px;">
                    <p style="margin:0;font-size:13px;color:#92400e;">
                      If you did not sign up for VeloChat, you can safely ignore this email.
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td align="center" style="background-color:#f9fafb;border-top:1px solid #e5e7eb;padding:20px 40px;">
              <p style="margin:0;font-size:12px;color:#9ca3af;">&copy; ${new Date().getFullYear()} VeloChat. All rights reserved.</p>
